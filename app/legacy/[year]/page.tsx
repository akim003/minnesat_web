                ...
                {project.gallery
                  ...
                  : // Fallback to placeholder images if no gallery is defined
                    Array.from({ length: 6 }).map((_, i) => (
                      <div key={i} className="relative h-64 overflow-hidden rounded-lg border border-white/10 group">
                        <Image
                          src={`/placeholder.svg?key=alkq6&key=bwaj3&key=qnme5&key=5vt0p&key=zw4af&key=t1vpo&key=zbhwz&key=lydov&height=300&width=400&query=cansat project image ${project.year} ${i + 1}`}
